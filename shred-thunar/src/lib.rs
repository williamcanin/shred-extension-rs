...
struct ThunarxMenuProviderIface {
  ...
  get_file_items: Option<unsafe extern "C" fn(*mut MenuProvider, *mut GtkWidget, *mut GList) -> *mut GList>,
  get_background_items: Option<unsafe extern "C" fn(*mut MenuProvider, *mut GtkWidget, *mut FileInfo) -> *mut GList>,
}
...
unsafe extern "C" fn thunar_get_file_items(
  ...
  unsafe {
    ...
    while !node.is_null() {
      ...
      if !fi.is_null() {
        let raw = thunarx_file_info_get_uri(fi);
        if !raw.is_null() {
    ...
    let item = thunarx_menu_item_new(name.as_ptr(), label, tip, icon.as_ptr());
...
unsafe extern "C" fn thunar_get_background_items(
  _provider: *mut MenuProvider,
  _window: *mut GtkWidget,
  file: *mut FileInfo,
) -> *mut GList {
  unsafe {
    ...
    let raw = thunarx_file_info_get_uri(file);
    if raw.is_null() {
      return ptr::null_mut();
    }
    ...
    let item = thunarx_menu_item_new(
      name.as_ptr(),
...
unsafe extern "C" fn thunarx_menu_provider_iface_init(iface: gpointer, _: gpointer) {
  unsafe {
    let iface = &mut *(iface as *mut ThunarxMenuProviderIface);
    iface.get_file_items = Some(thunar_get_file_items);
    iface.get_background_items = Some(thunar_get_background_items);
  }
}
...
pub extern "C" fn thunar_extension_initialize(module: gpointer) {
  unsafe {
    ...
    let iface_info = GInterfaceInfo {
      interface_init: Some(thunarx_menu_provider_iface_init),
      interface_finalize: None,
      interface_data: ptr::null_mut(),
    };
    g_type_module_add_interface(module, shred_type, thunarx_menu_provider_get_type(), &iface_info);
